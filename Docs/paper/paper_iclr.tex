...
\begin{itemize}[leftmargin=*,itemsep=2pt,topsep=2pt]
    ...
    (Section~\ref{sec:mdd}); results and the residual analysis are in
...
\begin{figure}[t]
  ...
  \caption{The six Gen1-C corruptions: normal (left) vs.\ corrupted (right) event frame.
  Mechanisms in Section~\ref{sec:mechanisms}; membrane signatures in Section~\ref{sec:theory}.}
  \label{fig:schematic}
\end{figure}
...
\begin{figure}[t]
  ...
  \end{tikzpicture}}
  \caption{The MDD pipeline (Section~\ref{sec:mdd}).}
  \label{fig:mdd-flow}
\end{figure}
...
\begin{table}[h]
  ...
  corruption at severities 1--5 (no retraining; clean baseline $0.358$), tabulated and plotted.}
...
\begin{table}[t]
  ...
  \textbf{(a)}~per-frame. \textbf{(b)}~per-$W{=}64$-window.}
...
\begin{table}[t]
  ...
  hardware, against the $50$\,ms/frame deadline (methodology in Appendix~\ref{app:compute}).}
  \label{tab:mdd-hw}
  \begin{tabular}{llcc}
    \toprule
...
\begin{figure}[t]
  ...
  \caption{Representation ablation, mean AUROC over all corruptions and severities.
  Per-corruption breakdown in Appendix~\ref{app:repr}.}
  \label{fig:repr-summary}
\end{figure}

\subsection{Branch decomposition and fusion ablation}
\label{sec:branches}
Table~\ref{tab:mdd-branch} (Appendix~\ref{app:mdd-branches}) shows each branch is corruption-specialized
(radius owns dilations, L4 owns timing, RCF/spatial own contractions); the plain calibrated \texttt{max}
sits \emph{below} the best single branch on both residuals, motivating the studentized fusion
...
\texttt{hot\_pixel} is at $1.00$ by $W{=}16$ (${\approx}0.16$\,s), and \texttt{event\_flood}
--- the slowest of the four detectable corruptions to converge (Table~\ref{tab:mdd-w64}) ---
reaches $0.98$ by $W{=}128$.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\linewidth]{01_window_sweep_fused_L5.png}
  \caption{Fused AUROC vs.\ aggregation window $W$ (L5). Per-branch specialization is in
  Table~\ref{tab:mdd-branch} (Appendix~\ref{app:mdd-branches}).}
  \label{fig:window}
\end{figure}
...
(iii)~All AUROC values are point estimates from a single split ($95\%$ bootstrap CIs for the
headline numbers are in Appendix~\ref{app:window}), computed at a fixed $64$-frame window
(Table~\ref{tab:mdd-w64}); the full-recording upper bound (Table~\ref{tab:window}) is optimistic,
not a per-frame rate. AUROC itself measures label-known separability, not the accuracy a fixed
deployment threshold would achieve without labels.
...
Membrane potential statistics, already computed as a byproduct of the spiking network's forward
pass, give an unsupervised OOD signal whose behavior is predicted by a closed-form theory of the
leaky integrator: MDD reaches high AUROC on four of the six Gen1-C corruptions, and the two
residuals are exactly the cases the theory flags as information-limited or invariant to the
trained network, not failures of the detector. The one open problem, \texttt{spatial\_dropout},
points to spatial-dispersion features and adaptive neurons as the natural next directions -- a
target the theory itself identifies, not one found by post-hoc search.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Fused MDD at severity~5: AUPR and FPR@95\%, per-frame and per-$W{=}64$-window (same
  branches/fusion as Table~\ref{tab:mdd}).}
  \label{tab:mdd-aupr}
\end{table}
...
\begin{table}[t]
  \centering\small
  \caption{Inference-time compute comparison across baselines.}
  \label{tab:hardware}
...
\begin{table}[t]
  \centering\small
  \caption{Fused AUROC ($W{=}64$, L5) as the RCF reference set and search are shrunk for
  embedded export (column definitions in the text above).}
...
factors, not clock speed, decide real-time feasibility without a neural accelerator.
...
\begin{table}[t]
  ...
  \end{tabularx}
\end{table}
Each corruption is grounded in a documented event-camera (DVS) non-ideality; the mapping from fault
to our histogram-level operation is exactly the ``Our op'' pairing described alongside each corruption
in Section~\ref{sec:corr-algos}. One related-work note not covered there: the stochastic event-removal
primitive behind \texttt{spatial\_dropout} is also used, in a different role, as a \emph{training}
augmentation by EventDrop~\citep{gu2021eventdrop}.
...
theoretical predictions: \texttt{cls\_head} wins on four of six individual corruptions, but
\texttt{full\_membrane} ($\varphi$) leads on the overall mean.

\begin{table}[t]
  \centering\small
  \caption{Representation ablation, per-corruption Mahalanobis AUROC averaged over all five
  severities; \textbf{bold} marks the best representation per corruption.}
...
Figure~\ref{fig:detcmp}), PCA-Mahalanobis and Mahalanobis lead; $k$NN and OCSVM fall
\emph{below} chance because a one-sided density score inverts on the contraction corruptions. The
means cluster near $0.5$ precisely because no \emph{single}-sided detector can handle both
dilations and contractions: the motivation for MDD.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\linewidth]{27_detector_comparison.png}
  \caption{Seven classical OOD detectors on static $\varphi$, mean AUROC over all
...
\begin{figure}[t]
  ...
  \caption{MDD hyperparameter sensitivity (fused AUROC, $W{=}64$, L5): \texttt{k\_pca},
  \texttt{k\_nn}, a random-subsample reference cap, and farthest-point sampling replacing it.}
...
\begin{figure}[t]
  ...
  \begin{tabular}{@{}cc@{}}
    \includegraphics[width=0.49\linewidth]{16_alpha_tradeoff.png} &
    \includegraphics[width=0.49\linewidth]{22_improvement_by_severity.png} \\
    {\scriptsize (a) $\alpha$ trade-off} &
    {\scriptsize (b) improved $-$ baseline AUROC} \\
  \end{tabular}
  \caption{Studentized fusion ($W{=}64$, L5). \textbf{(a)}~$\alpha$ trade-off (default
  $\alpha{=}0.5$). \textbf{(b)}~improved $-$ baseline AUROC per corruption $\times$ severity.}
...
that stay flat at every window (Table~\ref{tab:window}, Figure~\ref{fig:window} in the main text).

\begin{table}[t]
  \centering\small
  \caption{Fused MDD AUROC vs.\ aggregation window $W$ (frames) at severity~5. $W=1$ is
  per-frame, ``full'' is whole-recording; the $W{=}64$ point estimate is not repeated here
  since it already appears in Table~\ref{tab:mdd-w64}(b) -- only its CI is new information.}
  \label{tab:window}
  \resizebox{\linewidth}{!}{%
  \begin{tabular}{lcccccc}
    \toprule
    ...
    \midrule
    \texttt{hot\_pixel}        & 1.000 & 1.000 & $[1.000,1.000]$ & 1.000 & $1.000$ $[1.000,1.000]$ \\
    \texttt{event\_rate\_shift}& 0.855 & 0.885 & $[0.858,0.927]$ & 0.914 & $0.955$ $[0.914,0.989]$ \\
    \texttt{event\_flood}      & 0.551 & 0.778 & $[0.875,0.904]$ & 0.976 & $1.000$ $[1.000,1.000]$ \\
    \texttt{temporal\_jitter}  & 0.850 & 0.859 & $[0.836,0.898]$ & 0.882 & $0.952$ $[0.915,0.984]$ \\
    \texttt{spatial\_dropout}  & 0.560 & 0.572 & $[0.522,0.629]$ & 0.584 & $0.620$ $[0.537,0.700]$ \\
    \texttt{polarity\_flip}    & 0.540 & 0.573 & $[0.522,0.634]$ & 0.587 & $0.611$ $[0.528,0.690]$ \\
    \bottomrule
  \end{tabular}}
\end{table}
...
AUROC crosses the $0.85$ operating threshold at $W{=}64$ ($3.2$\,s decision latency) and reaches
$0.976$ by $W{=}128$ ($6.4$\,s); the full-recording ceiling ($1.00$) requires the whole
${\sim}730$-frame ($36.5$\,s) recording and is an optimistic upper bound, not a deployable operating
point. For a time-critical setting, $W{=}64$ is the practical knee of the curve: cutting the window
further trades a steep AUROC loss ($0.89\to0.78$ at $W{=}32$) for a $2\times$ latency win, while
widening it past $W{=}128$ buys little accuracy for a large further latency cost.
...
\begin{figure}[t]
  ...
  \caption{Zero-shot cross-corruption transfer: a clean-vs-\texttt{hot\_pixel} detector
  evaluated on each held-out corruption (val-only, disjoint from training).}
  \label{fig:crosscorr}
\end{figure}
...
\begin{figure}[t]
  ...
  \emph{(b)}~cosine of the mean-shift directions: each taxonomy class's members align with each
  other and sit near-orthogonal to the other classes (values in Section~\ref{sec:geometry}).}
  \label{fig:pca-drift}
