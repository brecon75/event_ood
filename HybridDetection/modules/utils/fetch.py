...
from modules.detection_hybrid import Module_Hybrid as hybrid_module


def fetch_model_module(config: DictConfig) -> pl.LightningModule:
    model_str = config.model.name
    if model_str == 'hybrid':
        return hybrid_module(config)
    raise NotImplementedError(f"Model '{model_str}' not supported. Only 'hybrid' is available.")
